...
from pyclust_eval.core._adg_operations import execute_graph, get_subgraph, topological_sort
from pyclust_eval.core._shared_processes import common_operations
from pyclust_eval.core._utils import sum_of_upper_triu, find_midpoint
from itertools import combinations
from scipy.spatial.distance import cdist
from pyclust_eval.index_specifics.representatives import *
from pyclust_eval.index_specifics.s import *
from pyclust_eval.index_specifics.FFT import fft
...
class CVIToolbox:
    def __init__(self, X, y):
        ...
        self.cvi_methods_list = {'ball_hall': self.ball_hall, 'banfeld_raftery': self.banfeld_raftery,
                                 ...
                                 'det_ratio': self.det_ratio, 'dunn': self.dunn, 'gdi12': self.gdi12,
                                 'gdi13': self.gdi13,
    ...
    def calculate_icvi(self, cvi='all'):
        """
        ...
        Raises:
            AssertionError: When the list of cvi provided includes cvis with typos or indexes not yet implemented.

        Returns:
            dict: The dictionary that contains all the computed indexes so far.
        """
        if type(cvi) is list:
            assert all(x in self.cvi_methods_list.keys() for x in cvi), ("provided indexes may be mispelled or not yet "
                                                                         "implemented")
        if cvi == 'all':
            for index in self.cvi_methods_list.keys():
                try:
                    self.cvi_results[index] = self.cvi_methods_list[index]()
                    print(f'{index}: {self.cvi_results[index]}')
    ...
    def calinski_harabasz(self):
        ...
        self.cvi_results["calinski_harabasz"] = (bgd/wgd) * ((self.X.shape[0]-len(set(self.y)))/ (len(set(self.y))-1))
