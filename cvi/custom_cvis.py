...
def sd_dis(x, y):
    ...
    #Compute Dmin
    Dmin = np.delete(np.array(list(centers_distances.values())), np.where(np.array(list(centers_distances.values()))==0)).min()
...
def c_index(x, y):
    ...
    S_w = {i:pdist(x[y==i]).sum() for i in clusters}
...
def gdi31(x, y):
    ...
    centers = {i:x[y==i].mean(axis=0) for i in clusters}
    #Compute deltas
    deltas = {(i,j):pairwise_distances(x[y==i], x[y==j]).sum() / (y[y==i].shape[0] * y[y==j].shape[0])  for i, j in combinations(clusters, r=2)}
    return min(deltas.values())/max(Deltas.values())
...
def gdi51(x, y):
    ...
    centers = {i:x[y==i].mean(axis=0) for i in clusters}
    #Compute deltas
    deltas = {(i,j):(np.linalg.norm(x[y==i] - centers[i], axis=1).sum() + np.linalg.norm(x[y==j] - centers[j], axis=1).sum()) / (y[y==i].shape[0] + y[y==j].shape[0])  for i, j in combinations(clusters, r=2)}
    return min(deltas.values())/max(Deltas.values())
...
def gdi61(x, y):
    ...
    Deltas = {i:pairwise_distances(x[y==i], x[y==i]).max() for i in clusters}
    #Compute deltas
    deltas = {(i,j):max(directed_hausdorff(x[y==i], x[y==j])[0], directed_hausdorff(x[y==j], x[y==i])[0])  for i, j in combinations(clusters, r=2)}
    return min(deltas.values())/max(Deltas.values())
...
def ratkowsky_lance(x, y):
    ...
    for j in range(BGSS.shape[0]):
        for cluster in clusters:
            BGSS[j] += y[y==cluster].shape[0] * (centers[cluster][j] - X_cen[j])**2
    #TSS = np.array([((x[:,j] - X_cen[j])**2).sum() for j in range(x.shape[1])])
    TSS = np.zeros((1, x.shape[1]))
    for j in range(TSS.shape[0]):
        for i in range(x.shape[0]):
            TSS[j] += (x[i,j] - X_cen[j])**2
    R_bar = (BGSS / TSS).mean()
...
def wemmert_gancarski(x, y):
    ...
    #Compute centers
    centers = {i:x[y==i].mean(axis=0) for i in clusters}
    R = [np.linalg.norm(x[i] - centers[y[i]]) / np.array([np.linalg.norm(x[i] - centers[j]) for j in clusters if j!=y[i]]).min() for i in range(x.shape[0])]
    R = np.array(R)
...
def density(x, y, centers, i, j):
    #Method used to compute s_dbw
    dens = 0
    if i==j:
        center = centers[i]
        std = np.sqrt(np.dot(np.var(x[y==i], 0), np.var(x[y==i], 0)))
    else:
        center = (centers[i] + centers[j]) / 2
    if i==j:
        total = x[y==centers[i]].shape[0]
        vectors = x[y==centers[i]]
        std = (np.sqrt(np.dot(np.var(x[y==i], 0), np.var(x[y==i], 0))) + np.sqrt(np.dot(np.var(x[y==j], 0), np.var(x[y==j], 0)))) / 2
    else:
        total = x[y==centers[i]].shape[0] + x[y==centers[j]].shape[0]
        vectors = x[np.logical_or(y==centers[i], y==centers[j])]
...
def s_dbw(x, y):
    ...
    for i in range(len(clusters)-1):
        for j in range(i+1, len(clusters)):
            Density += density(x, y, centers, clusters[i], clusters[j])
    Density *= 2 / (len(clusters) * (len(clusters) - 1))
    return sd_scat(x, y) + Density
...
def banfeld_raftery(x, y):
    ...
    s = 0
    for cluster in clusters:
        s += y[y==cluster].shape[0] * np.log((np.linalg.norm(x[y==cluster] - centers[cluster], axis=1)**2).sum() / y[y==cluster].shape[0])
    return s
...
def log_ss_ratio(x, y):
    ...
    #Compute centers
    centers = {i:x[y==i].mean(axis=0) for i in clusters}
    WGSS = np.array([np.array(np.linalg.norm(x[y==cluster] - centers[cluster], axis=1)**2).sum() for cluster in clusters]).sum()
    X_cen = x.mean(axis=0)
    BGSS = np.array([y[y==cluster].shape[0] * np.linalg.norm(centers[cluster] - X_cen)**2 for cluster in clusters]).sum()
    return np.log(BGSS / WGSS)
...
def det_ratio(x, y):
    ...
    #Compute within-cluster SSCP matrix
    W = T - B
    return np.linalg.det(T) / np.linalg.det(W)
...
def log_det_ratio(x, y):
    ...
    return x.shape[0] * np.log(np.linalg.det(T_mat) / np.linalg.det(WG_mat))
