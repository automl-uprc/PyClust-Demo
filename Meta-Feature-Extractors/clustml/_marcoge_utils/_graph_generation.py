...
def extract_edges(similarity_matrix, dataset_name="", threshold=0.9):
    ...
    if len(edge_list) == 0:
        ...
        return None
    path_to_write = fr"clustml\temp\edges\el_{dataset_name}.file"
...
def extract_node_embeddings(dataset_name):
    ...
    edge_list_path = fr"clustml\temp\edges\el_{dataset_name}.file"
    path_out = fr"clustml\temp\embeddings\ne_{dataset_name}.file"
...
def dataset_to_graph(dataset, dataset_name=""):
    ...
    distance threshold of the pairwise distances. Additionally, calculates node embeddings based on weighted deepwalk.
